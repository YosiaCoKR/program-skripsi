...
import { directionClass, directionOf, formatDelta, formatPercent } from "../lib/format";
...
export function IconCheck({ size = 14, className }: IconProps) {
  return (
    <svg width={size} height={size} viewBox="0 0 16 16" fill="none" className={className} aria-hidden="true">
      ...
      <path d="M5.2 8.2l2 2 3.6-4" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  );
}
...
export function IconAlertCircle({ size = 14, className }: IconProps) {
  return (
    <svg width={size} height={size} viewBox="0 0 16 16" fill="none" className={className} aria-hidden="true">
      <circle cx="8" cy="8" r="6.5" stroke="currentColor" strokeWidth="1.5" />
      <path d="M8 4.6v4.2" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" />
...
export function IconTriangle({ size = 14, className }: IconProps) {
  return (
    <svg width={size} height={size} viewBox="0 0 16 16" fill="none" className={className} aria-hidden="true">
      <path d="M8 2.2L14.5 13.5H1.5L8 2.2z" stroke="currentColor" strokeWidth="1.5" strokeLinejoin="round" />
      <path d="M8 6.4v3.1" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" />
...
export function IconOctagon({ size = 14, className }: IconProps) {
  return (
    <svg width={size} height={size} viewBox="0 0 16 16" fill="none" className={className} aria-hidden="true">
      ...
      />
      <path d="M6 6l4 4M10 6l-4 4" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" />
    </svg>
  );
}
...
export function IconDash({ size = 14, className }: IconProps) {
  return (
    <svg width={size} height={size} viewBox="0 0 16 16" fill="none" className={className} aria-hidden="true">
      ...
      <path d="M5.4 8h5.2" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" />
    </svg>
  );
}

export function IconArrow({ direction, size = 12 }: { direction: "up" | "down" | "flat"; size?: number }) {
  if (direction === "flat") {
    return (
 ...
 */
export function StatusBadge({ level, title }: { level: AlertLevel | string; title?: string }) {
  const style = statusStyle(level);
  const Icon = STATUS_ICONS[level as AlertLevel] ?? IconDash;
  return (
    <span className={`status ${style.className}`} title={title ?? style.description}>
...
export function DeltaValue({
  ...
  return (
    <span className={`row ${directionClass(direction)}`} style={{ gap: 4, fontWeight: 600 }}>
...
export function Loading({ label = "Memuat data…" }: { label?: string }) {
  return (
    <div className="stack-3" role="status" aria-live="polite">
...
export function Notice({
  ...
  const Icon = tone === "critical" ? IconOctagon : tone === "warning" ? IconTriangle : IconAlertCircle;
  return (
    ...
    <div className="grid grid-3">
      {metrics.map((metric) => (
        <div key={metric.label} className="stack-1">
          ...
          <div className="num" style={{ fontSize: "var(--text-md)", fontWeight: 600 }}>
            {metric.value}
          </div>
